...
def parse_arguments(DEFAULT_ORIENTATION):
...
TAP_MOVEMENT_THRESHOLD = 50
TAP_DURATION_THRESHOLD = 0.12
SCROLL_MOVEMENT_THRESHOLD = 50
...
try:
    # load rotation args
    args = parse_arguments(DEFAULT_ORIENTATION)
    ...
    capture_report(touch_dev_name, max_x, max_y, args.rotate)
            ...
            if event.value == 1:  # Touch down
                ...
                if touch_start_time is not None:
                    ...
                    if finger_tap:
                        ...
                        if len(active_touches) <= 1:
                            print("one-finger tap")
                            device.emit(uinput.BTN_LEFT, 1)
                device.emit(uinput.BTN_LEFT, 0)
                three_finger_down = False
                active_touches.clear()
except KeyboardInterrupt:
    subprocess.run(["hyprctl", "keyword", f"device[{touch_dev_name}]:enabled", "1"])
